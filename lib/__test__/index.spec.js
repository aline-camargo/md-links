...
describe('Testando mdLinks', () => {
  ...
  test('Retorna erro quando passado arquivo .md válido não contendo links', (done) => {
    mdLinks('lib/__test__/__mocks__/teste-sem-links.md')
      .catch((response) => {
        expect(response).toMatch('Arquivo não contém links.');
        done();
      });
  });
  ...
  test('Retorna erro quando passado arquivo .md inválido', (done) => {
    mdLinks('lib/__test__/__mocks__/teste.md')
      .catch((response) => {
        expect(response).toMatch('Erro: arquivo não encontrado.');
        done();
      });
  });

  test('Retorna erro quando não passado nenhum arquivo', (done) => {
    mdLinks()
      .catch((response) => {
        expect(response).toMatch('Nenhum arquivo fornecido, nenhum arquivo analisado.');
        done();
      });
  });
  ...
  test('Retorna erro quando passado um diretório', (done) => {
    mdLinks('lib/__test__/__mocks__')
      .catch((response) => {
        expect(response).toMatch('Erro: ação inválida, forneça um arquivo ao invés de um diretório.');
        done();
      });
  });

  test('Retorna erro quando passado um diretório', (done) => {
    mdLinks('lib/__test__/index.spec.js')
      .catch((response) => {
        expect(response).toMatch('Erro: forneça um arquivo de extensão .md');
        done();
      });
  });
